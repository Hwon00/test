...
/*
  ...
  3. @WebsServlet 설정을 통한 서블릿 접근 방법은 서블릿 3.0부터 지원합니다.
  더 이상 web.xml에 서블릿 매핑에 관한 설정을 할 필요가 없습니다.
 
  4.@WebsServlet 애노테이션은 urlPatterns 속성을 값으로 갖는데 이 속성은
  해당 서블릿과 매핑될 URL 패턴을 지정할 때 사용됩니다.
  *@WebsServlet(urlPatterns="/LifeCycle")의 의미는
  "http://localhost:8088/프로잭트 이름/LifeCycle"로 요청이 들어오면
  실행할 서블릿이라는 의미입니다.
  
  5. urlPatterns은 여러 개의 url을 설정하기 위해 String 배열을 사용합니다.
  예) @WebsServlet(urlPatterns="/a")
  예) @WebsServlet(urlPatterns={"/a"})
  예) @WebsServlet(urlPatterns={"/a","/b","/c"})
  */

//@WebServlet(urlPatterns= {"/LifeCycle","/a"})
public class LifeCycle설명 extends HttpServlet{
//	private static final long serialVersionUID = 8535560556802906320L;
/*
 1. serialVersionUID는 같은 클래스임을 알려주는 식별자 역할을 하며
 	...
 	역직렬화(byte로 변환된 데이터를 원래의 객체로 변환하는 기술) 실행될 때
 	자동으로 부여됩니다.
 	
 	3. 역직렬화 시 직렬화 기능 클래스의 내용이 그대로이면 같은 번호가 부여되고
 ...
 * */
	...
	/*
	 클라이언트의 요청이 있을 때 마다 실행됩니다.
	 service()메소드가 끝나면 서버에서의 실행은 끝납니다.
	 서버 프로그램 실행이 완료된 후에는 서블릿 컨테이너가 실행 결과를 웹 서버에 전달하고
	 웹 서버는 서비스를 요청한 클라이언트에 응답합니다.
	 이로써 웹에서 하나의 요청에 대한 처리가 완료됩니다.
	 
	 HTTP 메소드를 참조하여 doGet()을 호출할 지, doPost()를 호출할 지 결정하며
	 요청이 있을 때마다 호출됩니다.
	 브라우저에서 새로고침을 해보세요.
	  */
